...
const ExtendInfo: React.FC<ExtendInfoProps> = ({ sessionId }) => {
  ...
  if (isLoading) {
    return (
      <div className="webchat-extend-info-loading">
        <Spin size="small" />
        <span>加载中...</span>
      </div>
    );
  ...
  if (extendInfo.length === 0) {
    return (
      <div className="webchat-extend-info-empty">请选择会话查看扩展信息</div>
    );
  }

  return (
    <div className="webchat-extend-info">
      <List
        size="small"
        dataSource={extendInfo}
        renderItem={(item) => (
          <List.Item>
            <div className="webchat-extend-item">
