...
const ChatInfoPanel: React.FC<ChatInfoPanelProps> = ({ sessionId }) => {
  ...
  if (isLoading) {
    return (
      <div className="webchat-chat-info-loading">
        <Spin size="small" />
        <span>加载中...</span>
      </div>
    );
  ...
  if (!chatInfo) {
    return (
      <div className="webchat-chat-info-empty">请选择会话查看对话信息</div>
    );
  }

  return (
    <div className="webchat-chat-info">
      <Descriptions column={1} size="small">
        ...
        <Descriptions.Item label="消息数">
          <div className="webchat-message-count">
